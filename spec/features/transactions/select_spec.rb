...
describe "Transaction selection", :js do
  ...
  let(:inflow)     { create(:transaction, budget: budget, account: account, amount: 2_500) }
  let(:outflow)    { create(:transaction, budget: budget, account: account, amount: -1_000) }
  let(:scheduled)  { create(:transaction, :recurring, budget: budget, account: account, amount: -500) }
  ...
  def selected_total(amount, count)
    "#{amount}\n#{t("transactions.summaries.show.selected_total", count: count)}"
  end
end
