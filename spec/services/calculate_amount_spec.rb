...
describe CalculateAmount do
  describe ".call" do
    ...
    context "when the expression multiplies by a zero right operand" do
      let(:expression) { "100*0" }

      it { is_expected.to eq(Money.from_amount(BigDecimal("100"))) }
    end

    context "when the expression divides by a zero right operand" do
      let(:expression) { "10/0" }
    ...
    context "when the expression is comma-grouped in a multiplication" do
      let(:expression) { "1,000*2" }

      it { is_expected.to eq(Money.from_amount(BigDecimal("2000"))) }
    end
    ...
    context "when the expression is a multiplication" do
      let(:expression) { "10.50*2" }

      it { is_expected.to eq(Money.from_amount(BigDecimal("21"))) }
    end

    context "when the expression is a division" do
      let(:expression) { "100.00/4" }

      it { is_expected.to eq(Money.from_amount(BigDecimal("25"))) }
    end

    context "when the expression divides into a repeating decimal" do
      let(:expression) { "100/3" }

      it { is_expected.to eq(Money.from_amount(BigDecimal("33.33"))) }
    end
    ...
    context "when the expression has whitespace around a multiplication operator" do
      let(:expression) { "10 * 5" }

      it { is_expected.to eq(Money.from_amount(BigDecimal("50"))) }
    end

    context "when the expression has whitespace around a division operator" do
      let(:expression) { "10 / 5" }

      it { is_expected.to eq(Money.from_amount(BigDecimal("2"))) }
    end
    ...
    context "when the expression multiplies a negative left operand" do
      let(:expression) { "-50*2" }
    ...
    context "when the expression has a trailing multiplication operator" do
      let(:expression) { "10*" }

      it { is_expected.to eq(Money.from_amount(BigDecimal("10"))) }
    end

    context "when the expression has a trailing division operator" do
      let(:expression) { "10/" }
    ...
    context "when the expression multiplies by invalid decimal parts" do
      let(:expression) { "100*.." }

      it { is_expected.to eq(Money.from_amount(BigDecimal("100"))) }
    end

    context "when the expression divides by invalid decimal parts" do
      let(:expression) { "100/.." }

      it { is_expected.to eq(Money.from_amount(BigDecimal("100"))) }
    end

    context "when the expression has a signed right operand" do
      let(:expression) { "2*-3" }
    ...
    context "when the expression chains a multiplication and a division" do
      let(:expression) { "100*10/5" }
